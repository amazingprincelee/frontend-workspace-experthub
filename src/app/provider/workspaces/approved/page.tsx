...
import { WorkspaceCard } from "@/components/WorkspaceCard"; // Import WorkspaceCard from the dashboard
...
const ApprovedWorkspaces: React.FC = () => {
  ...
  const fetchProviderWorkspaces = async () => {
    try {
      ...
      if (!response.data.approvedWorkspaces) {
      ...
      const approved: Workspace[] = response.data.approvedWorkspaces.map((workspace: any) => ({
        ...
        approved: workspace.approved !== undefined ? workspace.approved : true,
      }));
    ...
    <ProtectedRoute allowedRoles={["provider"]}>
      <div className="container mx-auto px-4 sm:px-6 py-6 bg-background min-h-screen">
        <h2 className="text-lg sm:text-xl font-heading font-bold text-primary mb-4">
          All Approved Workspaces
        </h2>
        {approvedWorkspaces.length === 0 ? (
          <p className="text-gray text-center py-4">No approved workspaces found</p>
        ) : (
